...
Assert-Match $js 'items\.forEach\(\(item\) => workUnderlineObserver\.observe\(item\)\)' "Each work item must share the underline observer"
Assert-Match $js 'const revealWorkTick = \(item\) => \{[\s\S]*item\.classList\.add\("is-ticked"\)' "Work ticks must reveal per list item"
...
Assert-Match $css '\.work-card li\.is-ticked\s+\.work-tick__path\s*\{[\s\S]*stroke-dashoffset:\s*0' "Work tick state must belong to each list item"
...
Assert-NotMatch $css '--tick-delay|transition-delay:\s*var\(--tick-delay' "Legacy time-based tick cascade must be removed"

...
Assert-PathExists (Join-Path $root "assets\cases\yoga-qigong-telegram-ads-1.webp") "Yoga case image missing"
Assert-PathExists (Join-Path $root "assets\cases\medcenter-telegram-ads-1.webp") "Medcenter case image missing"
...
$generatorPath = Join-Path $root "scripts/generate-cases.js"
Assert-PathExists $generatorPath "Static case generator is missing"
...
Assert-Match $homeJs "if \(!canTiltCaseCard\(\) \|\| mobileCaseViewport\.matches\) return;" "Desktop count-up must retain the pointer gate and skip the mobile observer path"
Assert-Match $homeJs "if \(prefersReducedMotion\.matches \|\| \(!mobileCaseViewport\.matches && !canTiltCaseCard\(\)\)\) return;" "Count-up preparation must retain reduced motion and input-mode gates"
...
$caseHeroMobileHeadingPattern = "@media \(max-width:\s*560px\)[\s\S]*?\.case-hero h1\s*\{[\s\S]*?width:\s*100%;[\s\S]*?min-width:\s*0;[\s\S]*?max-width:\s*100%;[\s\S]*?font-size:\s*var\(--case-title-font-size, 64px\);"
$caseHeroMobileGridPattern = "@media \(max-width:\s*560px\)[\s\S]*?\.case-hero\s*\{[\s\S]*?grid-template-columns:\s*minmax\(0, 1fr\);"
...
$caseHeroMobileWordPattern = "@media \(max-width:\s*560px\)[\s\S]*?\.case-hero h1 \.text-roll-word\s*\{[\s\S]*?max-width:\s*none;[\s\S]*?white-space:\s*nowrap;[\s\S]*?overflow-wrap:\s*normal;"
...
Assert-Match $css $caseHeroMobileHeadingPattern "Mobile case hero must use the full available width and adaptive font size"
Assert-Match $css $caseHeroMobileGridPattern "Mobile case hero grid must allow long titles to shrink to the available width"
Assert-Match $css $caseHeroMobileRollPattern "Mobile case hero text roll must stay inside the heading width"
Assert-Match $css $caseHeroMobileWordPattern "Mobile case hero words must stay whole"
Assert-Match $homeJs 'function\s+fitMobileCaseTitle\s*\(' "Adaptive mobile case title fitter is missing"
Assert-Match $homeJs 'CASE_TITLE_MIN_FONT_SIZE_PX\s*=\s*32' "Mobile case title minimum font size must be 32px"
Assert-Match $homeJs 'CASE_TITLE_MAX_FONT_SIZE_PX\s*=\s*64' "Mobile case title maximum font size must be 64px"
Assert-Match $homeJs 'document\.fonts\.ready\.then\(queueCaseTitleFit\)' "Mobile case titles must be refitted after fonts load"
Assert-Match $homeJs 'addEventListener\("resize",\s*queueCaseTitleFit\)' "Mobile case titles must be refitted after viewport changes"

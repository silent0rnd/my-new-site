...
Assert-Contains $styles 'min-height: 72svh;' "Footer missing stable desktop height"
...
Assert-Contains $styles '.is-signature-pending .site-footer__signature-image' "Early hidden state for the signature is missing in styles"
Assert-Contains $script 'signaturePendingRoot.classList.remove("is-signature-pending")' "Signature reveal must clear the early hidden state"
